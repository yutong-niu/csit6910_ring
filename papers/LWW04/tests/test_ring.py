...
class RingTest(TestCase):
    def test_LSAG_verification(self):
        ...
        for i in range(size):
            ...
            self.assertTrue(signature_1[-1] == signature_2[-1])
            self.assertFalse(signature_1[-1] == signature_3[-1])
            self.assertFalse(signature_2[-1] == signature_3[-1])
    ...
    def test_LSAG_linkability(self):
        ...
        for i in range(size):
            ...
            self.assertTrue(signature_1[-1] == signature_2[-1])
            self.assertTrue(signature_3[-1] == signature_4[-1])
            self.assertFalse(signature_1[-1] == signature_3[-1])
            self.assertFalse(signature_2[-1] == signature_3[-1])